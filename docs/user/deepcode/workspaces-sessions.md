...
Select a project folder from the DeepCode home page before creating a session. The workspace becomes the default working directory for the agent and its tools.
...
DeepCode stores sessions through Harness in the active Harness Home. Reopen a previous session from the session list to continue with its recorded conversation and events.

Changing the active Profile or Harness Home changes which session store the interface reads. Returning to the original Home and Profile makes its sessions available again; DeepCode does not copy sessions between Homes.
...
Attachments become model input only through Harness. DeepCode does not keep a second attachment database.
...
An interrupted running turn does not erase the saved session history. The current operation may stop when Harness restarts or DeepCode quits, but the recorded conversation remains on disk.

## Window and tray behavior

Closing the main window hides DeepCode to the system tray. Harness and any current task continue running. **Quit DeepCode** stops Harness and can interrupt the active task, so DeepCode asks for confirmation before exiting.
