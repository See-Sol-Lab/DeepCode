...
新建会话前，先从 DeepCode 首页选择项目文件夹。该工作区会成为 agent 及其工具的默认工作目录。
...
DeepCode 通过 Harness 把会话保存在当前 Harness Home 中。从会话列表重新打开旧会话，即可继续使用已记录的对话与事件。

切换当前 Profile 或 Harness Home 后，界面会读取另一份会话存储。回到原来的 Home 与 Profile 后，对应会话会重新出现；DeepCode 不会在不同 Home 之间复制会话。

## 附加文件与图片

所选模型支持图片输入时，可以附加图片。需要检查项目文件时，请告诉 agent 对应路径，不要把大型文件直接粘贴进对话。

附件只会通过 Harness 进入模型输入。DeepCode 不维护第二份附件数据库。

## 检查 agent 的工作

要求 agent 概述修改过的文件与验证结果。源代码改动请使用版本控制进行 review（审查），并在允许工作区日常流程之外的操作前检查工具批准请求。

正在运行的一轮被中断时，已保存的会话历史不会消失。Harness 重启或 DeepCode 退出可能停止当前操作，但已记录的对话仍保留在磁盘上。

## 窗口与托盘行为

关闭主窗口会把 DeepCode 隐藏到系统托盘。Harness 与当前任务会继续运行。**退出 DeepCode**会停止 Harness，并可能中断当前任务，因此 DeepCode 会在退出前要求确认。
