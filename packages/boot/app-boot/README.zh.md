...
| `inspectExistingProfile` / `inspectExistingProfiles` / `classifySurface` / `WEB_SURFACE_ROWS` / `HEADLESS_SURFACE_ROWS` | boot-free、只读的 profile 检查与静态 surface 分类（见 [Profile](#profiles)） |
