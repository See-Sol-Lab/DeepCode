...
[可移植发行目录](2026-08-16-desktop-portable-distribution.zh.md)可以从文件夹运行，但非程序员仍然没有安装体验：没有开始菜单或桌面快捷方式、没有卸载入口，而且打包应用与机器上其他所有 dsh 共享全局 `~/.dsh`。本里程碑增加按用户安装的安装程序，并让应用拥有自己的数据目录，使凭据、设置与会话清晰且可清除。
