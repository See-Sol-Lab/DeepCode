...
- **Diagnostics for GUI users**: the packaged GUI pipes DSH stdout/stderr into `%APPDATA%\DeepSeekGUI\dsh-service.log` (previous run rotated to `.old`, 5MB cap with a truncation marker, `sk-…` shapes redacted before write); error dialogs point at that log instead of a terminal. Dev and smoke keep inheriting the console.
