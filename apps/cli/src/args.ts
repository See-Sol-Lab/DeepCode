...
/** Boot-free, read-only profile inspection for machine consumers. */
interface ProfilesInvocation {
  mode: 'profiles'
}

/** The resolved `dsh` invocation. Help, version, and errors exit inside {@link parseDshArgs}. */
export type DshInvocation = ProfileInvocation | DumpConfigInvocation | PluginInvocation | ProfilesInvocation
...
Examples:
  ...
  dsh profiles --json                        list every profile and its static surface, without booting
`
...
export function parseDshArgs(argv: readonly string[], version: string): DshInvocation {
  ...
  const profiles = program.command('profiles').description('list the Harness home profiles as boot-free, read-only JSON (stdout carries only the JSON document)')
  profiles
    .option('--json', 'print the machine-readable JSON document (schemaVersion 1)')
    .action((options: { json?: boolean }) => {
      rejectParentOptions('profiles')
      if (options.json !== true) program.error('error: profiles needs --json')
      resolved = { mode: 'profiles' }
    })
