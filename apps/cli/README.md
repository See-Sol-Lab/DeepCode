...
| `dsh profiles --json` | List every real profile and its static surface classification as one JSON document on stdout, boot-free and read-only. |
...
Use `--dump-default-config` and `--dump-config` to inspect the composed tree without booting it. `dsh profiles --json` classifies every real profile statically — `web-capable`, `headless`, `candidate`, or `malformed` — over the same layer order (bundle patches, the profile's `cordis.patch.yml`, then the home-level `$DSH_HOME/cordis.patch.yml`) without initializing, healing, or writing anything.
