...
describe.skipIf(!existsSync(dshBin))('dsh BUILT bin (node lib/bin.js, no tsx)', () => {
  ...
  describe('profiles', () => {
    let home: string
    beforeEach(() => { home = mkdtempSync(join(tmpdir(), 'dsh-profiles-bin-')) })
    afterEach(() => { rmSync(home, { recursive: true, force: true }) })

    const stage = (name: string, bundles: string[], patch = '[]\n'): void => {
    ...
    it('prints only the JSON document and creates nothing for a missing home', async () => {
      const { stdout, code, stderr } = await runBuiltBin(['profiles', '--json'], {
        DSH_HOME: home,
        DEEPSEEK_API_KEY: 'sk-profiling-secret',
      })
      expect(code).toBe(0)
      expect(stderr).toBe('')
      const parsed = JSON.parse(stdout) as { schemaVersion: number; dshHome: string; profiles: unknown[] }
      expect(parsed).toEqual({ schemaVersion: 1, dshHome: home, profiles: [] })
      expect(stdout).not.toContain('sk-profiling-secret')
      expect(existsSync(join(home, 'profiles'))).toBe(false)
    }, 30_000)

    it('classifies real web, headless, custom, and broken profiles without writing to them', async () => {
      stage('web', ['@deepseek-ai/dsh-base', '@deepseek-ai/dsh-web-app'])
      stage('headless', ['@deepseek-ai/dsh-base', '@deepseek-ai/dsh-headless'])
      stage('custom', ['@deepseek-ai/dsh-base'])
      stage('broken', ['@deepseek-ai/dsh-ghost'])
      const webManifest = readFileSync(join(home, 'profiles', 'web', 'package.json'), 'utf8')
      const { stdout, code, stderr } = await runBuiltBin(['profiles', '--json'], { DSH_HOME: home })
      expect(code).toBe(0)
      const parsed = JSON.parse(stdout) as { profiles: { name: string; staticStatus: string }[] }
      expect(parsed.profiles.map(profile => [profile.name, profile.staticStatus])).toEqual([
        ['broken', 'malformed'],
        ['custom', 'candidate'],
        ['headless', 'headless'],
        ['web', 'web-capable'],
      ])
      // 诊断（malformed 原因）只进 stderr；stdout 里只有 JSON 文档。
      expect(stderr).toContain('cannot resolve profile bundle')
      expect(readFileSync(join(home, 'profiles', 'web', 'package.json'), 'utf8')).toBe(webManifest)
      expect(existsSync(join(home, 'profiles', 'web', 'cordis.yml'))).toBe(false)
    }, 30_000)

    it('reports unmatched patch targets on stderr while stdout stays a pure JSON document', async () => {
      stage('web', ['@deepseek-ai/dsh-base', '@deepseek-ai/dsh-web-app'], '- id: no-such-row\n  config:\n    x: 1\n')
      const { stdout, code, stderr } = await runBuiltBin(['profiles', '--json'], { DSH_HOME: home })
      expect(code).toBe(0)
      const parsed = JSON.parse(stdout) as { profiles: { name: string; staticStatus: string }[] }
      expect(parsed.profiles.map(profile => [profile.name, profile.staticStatus])).toEqual([['web', 'web-capable']])
      expect(stderr).toContain('"no-such-row" not found')
    }, 30_000)

    it('a broken home patch with a test key redacts the key from stderr and the JSON error', async () => {
      const key = 'sk-builtbin-profiles-key123456'
      stage('web', ['@deepseek-ai/dsh-base', '@deepseek-ai/dsh-web-app'])
      writeFileSync(join(home, 'cordis.patch.yml'), `- id: webserver\n  config: {broken\n# ${key}\n`)
      const { stdout, code, stderr } = await runBuiltBin(['profiles', '--json'], { DSH_HOME: home })
      expect(code).toBe(0)
      expect(stderr).not.toContain(key)
      expect(stdout).not.toContain(key)
      const parsed = JSON.parse(stdout) as { profiles: { staticStatus: string; error?: string }[] }
      expect(parsed.profiles[0]!.staticStatus).toBe('malformed')
      expect(parsed.profiles[0]!.error).toContain('failed to parse')
    }, 30_000)
  })
