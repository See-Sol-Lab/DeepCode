...
| `dsh profiles --json` | 以 boot-free、只读的方式，把每个真实 profile 及其静态 surface 分类输出为 stdout 上的一份 JSON 文档。 |
...
使用 `--dump-default-config` 和 `--dump-config` 可在不启动的情况下检查组合后的配置树。`dsh profiles --json` 按与启动一致的层序（组合包 patch、profile 自身的 `cordis.patch.yml`、home 级 `$DSH_HOME/cordis.patch.yml`）对每个真实 profile 做静态分类——`web-capable`、`headless`、`candidate` 或 `malformed`——不初始化、不修复、不写入任何东西。
