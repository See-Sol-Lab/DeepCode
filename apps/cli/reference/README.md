...
`dsh profiles --json` inspects every real profile under the Harness home without booting and prints exactly one JSON document on stdout; skipped-patch and malformed diagnostics go to stderr only. A missing home or `profiles` directory prints an empty list and creates nothing — the command is strictly read-only: it never initializes profiles, heals the module fallback, or writes manifests, patch files, or `cordis.yml`.

```sh
dsh profiles --json
```

The document is `schemaVersion: 1` with the resolved `dshHome` and one entry per profile directory (the maintained `profiles/node_modules` fallback and non-directories are skipped, and a missing home patch layer counts as empty): `name`, `dir`, `bundles` in manifest order, `staticStatus`, `evidence`, and — for a `malformed` entry only — `error`. Classification is static over the same layer order a boot applies: each bundle patch, the profile's own `cordis.patch.yml`, then the home-level `$DSH_HOME/cordis.patch.yml`.

The four statuses:

- `web-capable` — the official `web-startup`, `webserver`, `web-runtime` rows are present with their official plugin names (`@deepseek-ai/dsh-web-app/startup`, `@deepseek-ai/dsh-host-webserver`, `@deepseek-ai/dsh-web-app`) and none is literally `disabled: true`.
- `headless` — the official `headless-startup`, `headless-runner` rows (`@deepseek-ai/dsh-headless/startup`, `@deepseek-ai/dsh-headless`) are present and not literally disabled.
- `candidate` — official rows are literally disabled, dynamically (`!!js`) disabled, reused under custom plugin names, or absent; a custom surface is never guessed.
- `malformed` — that profile's inspection threw (unreadable manifest, unresolvable bundle, unparsable patch); the credential-redacted reason is its `error`, and the other profiles still report.

One broken profile or a broken home patch layer never fails the whole discovery, and a profile whose boot would fail is not classified — that belongs to a real launch. Credential-shaped text in failure messages is redacted before it reaches stdout or stderr.
