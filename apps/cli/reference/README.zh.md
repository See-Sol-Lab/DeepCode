...
`dsh profiles --json` 在不启动的情况下检查 Harness home 下的每个真实 profile，并在 stdout 上打印且仅打印一份 JSON 文档；跳过 patch 与 malformed 的诊断只进 stderr。home 或 `profiles` 目录缺失时输出空列表，且不创建任何东西——该命令严格只读：绝不初始化 profile、修复模块后备目录，也不写入 manifest、patch 文件或 `cordis.yml`。

```sh
dsh profiles --json
```

文档为 `schemaVersion: 1`，含解析出的 `dshHome`，以及每个 profile 目录对应的一条记录（跳过维护用的 `profiles/node_modules` 后备目录与非目录条目；缺失的 home patch 层视为空层）：`name`、`dir`、按 manifest 顺序的 `bundles`、`staticStatus`、`evidence`，以及仅 `malformed` 条目才有的 `error`。分类是静态的，按与真实启动一致的层序组合：各组合包 patch、profile 自身的 `cordis.patch.yml`、home 级的 `$DSH_HOME/cordis.patch.yml`。

四种状态：

- `web-capable`——官方 `web-startup`、`webserver`、`web-runtime` 行齐全且插件名为官方值（`@deepseek-ai/dsh-web-app/startup`、`@deepseek-ai/dsh-host-webserver`、`@deepseek-ai/dsh-web-app`），且没有任何一行的字面 `disabled: true`。
- `headless`——官方 `headless-startup`、`headless-runner` 行（`@deepseek-ai/dsh-headless/startup`、`@deepseek-ai/dsh-headless`）齐全且无字面禁用。
- `candidate`——官方行被字面禁用、动态（`!!js`）禁用、被自定义插件名复用，或缺失；自定义 surface 绝不猜测。
- `malformed`——该 profile 的检查抛出错误（manifest 不可读、bundle 无法解析、patch 无法解析）；已做凭据脱敏的原因是它的 `error`，其他 profile 照常报告。

单个损坏 profile 或损坏的 home patch 层绝不拖垮整个发现；启动会失败的 profile 也不参与分类——那属于真实启动的职责。失败消息中的凭据形态文本在进入 stdout 或 stderr 之前会被脱敏。
